...
class DocumentRepositories {
  ...
  async findById(id) {
    ...
    // Ambil hasil ekstraksi terbaru (jika ada)
    if (rows[0].job_id) {
      ...
      if (resultRows.length) {
        ...
        const { rows: fields } = await pool.query(
          'SELECT key, value FROM fields WHERE extraction_result_id = $1', [resultId]
        );
        const { rows: itemRows } = await pool.query(
          `SELECT id, row_index FROM items WHERE extraction_result_id = $1
           ORDER BY row_index`, [resultId]
        );

        const items = [];
        for (const item of itemRows) {
          const { rows: colRows } = await pool.query(
            'SELECT key, value FROM item_fields WHERE item_id = $1', [item.id]
          );
          items.push({ row_index: item.row_index, columns: colRows });
        }
  ...
  #mapRow(row) {
    return {
      id: row.id, start_page: row.start_page, end_page: row.end_page,
      status: row.status, confidence: row.confidence,
      needs_review: row.needs_review, error_message: row.error_message,
      created_at: row.created_at, updated_at: row.updated_at,
      job_id: row.job_id,
      document_type: row.dt_id ? { id: row.dt_id, code: row.dt_code, name: row.dt_name } : null,
